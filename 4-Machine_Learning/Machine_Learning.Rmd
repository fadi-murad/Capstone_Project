...
All 3 models almost have the same AUC *0.92*, original task has the best tpr while under and over tasks have the best tnr  
let us train all of tasks and check their performance  
...
Let us visualize the result the performance by ploting confusion matrixs and ROC curves  
...
conf_Mat <- function(pre,con_title){
  aa<- as.data.frame(table(f_test$income_level,pre$data$response)) 
...
I am gonna train both under and over tasks to compare their performance on the test set as I suspect overfitting for over-sampling task in one of the classes
